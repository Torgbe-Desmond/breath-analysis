...
const createQuestion = async (req, res, next) => {
  ...
  try {
    ...
    // Invalidate Redis caches (best-effort; ignore if Redis is down)
    try {
      if (redisClient.isOpen && results.data?.length) {
        const uniqueCategoryIds = [
          ...new Set(results.data.map((r) => String(r.categoryId))),
        ];
        const pipeline = redisClient.multi();
        for (const categoryId of uniqueCategoryIds) {
          pipeline.del(`insights_${categoryId}`);
        }
        pipeline.del("questions");
        await pipeline.exec();
      }
    } catch (cacheErr) {
    ...
    try {
      if (redisClient.isOpen) {
        ...
        if (cached) {
          ...
          const totalPages = Math.ceil(cachedData.totalQuestions / limit) || 1;
      ...
      console.warn("Redis read failed:", cacheReadErr.message);
    }

    // Cache miss → compute
    const results = await questionService.getCategoryInsights(categoryId);

    if (!results || results.totalQuestions === undefined) {
      return res
        .status(404)
        .json(new ResponseModel(null, "Category insights not found", 404));
    }

    const totalPages = Math.ceil(results.totalQuestions / limit) || 1;
    ...
    try {
      if (redisClient.isOpen) {
        ...
        if (cachedCount >= MAX_CACHED_CATEGORIES) {
          const leastVisited = await redisClient.zRange(
            "cached_categories",
            0,
            0,
          );
      ...
      console.warn("Redis write failed:", cacheWriteErr.message);
    }
